...
function calculatePAYE() {
    ...
    //const nssf = Math.min((basicAmount + allowances + irrgularallowances) * 0.06, 4320); // Updates From Feb 2026 Payroll
    const nssf = Math.min((basicAmount + allowances + irrgularallowances) * 0.06, 6480);
    ...
    const taxabletelnonCashBenefits = telnonCashBenefits <= 5000 ? 0 : telnonCashBenefits; //If telnonCashBenefits is less than or equal to 3000, the taxable amount is 0. -----   Otherwise, the taxable amount is telnonCashBenefits.
    const taxablemealsnonCashBenefits = mealsnonCashBenefits <= 5000 ? 0 : mealsnonCashBenefits;
    const allowableDeductions = Math.min(nssf + pension, 30000) + housingLevy + shif;
    //{SHIF & AHL deduction have qualified to be ALLOWABLE DEDUCTIONS}
    const excessPension = Math.max(((pensionEmployer + nssf) - 30000), 0);
    ...
    //const housingLevyRelief = Math.min(0.15 * housingLevy, 9000); {CEASES TO APPLY FROM DEC 27TH, 2024 ONWARDS}
    ...
    //const nssfEmployer = Math.min((basicAmount + allowances +irrgularallowances) * 0.06, 4320); // Updates as from Feb 2026
    const nssfEmployer = Math.min((basicAmount + allowances +irrgularallowances) * 0.06, 6480);
